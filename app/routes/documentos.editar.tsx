...
import { getDocumentById, saveDocumentToDb } from '~/services/documentService';
import { generatePDF } from '~/utils/pdfGenerator';

export function meta() {
  return [
    { title: 'Editar Documento - Yzen' },
    { name: 'description', content: 'Editar documento existente' },
  ];
}
...
export default function EditarDocumentoPage() {
  ...
  const updateField = <K extends keyof DocumentData>(field: K, value: DocumentData[K]) => {
  ...
  const updateModulo = (modId: string, field: keyof ModuloData, value: string | string[]) => {
  ...
  const removeModulo = (modId: string) => {
    if (formData.modulos.length > 1) {
      updateField('modulos', formData.modulos.filter((m) => m.id !== modId));
    }
  };
  ...
  const handleSave = async () => {
    await saveDocumentToDb(formData);
    navigate('/');
  };
  ...
  return (
    <div className="animate-fade-in max-w-5xl">
      ...
      <div className="mb-8">
        ...
        <h1 className="section-title">Editar Documento</h1>
        <p className="section-subtitle">Modifica los datos del documento</p>
      </div>
